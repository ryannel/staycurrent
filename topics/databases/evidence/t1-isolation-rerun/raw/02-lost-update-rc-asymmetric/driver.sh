...
#    new_balance = read_value - <its own amount> CLIENT-SIDE, the way a naive app would.
#    Amounts are DELIBERATELY ASYMMETRIC -- 40 and 30 from a starting 100 -- so the
#    final balance cannot be mistaken for a correct one. The round-3 version of this
#    experiment used 1000-100-100, whose result of 900 reads exactly like one correct
#    withdrawal; that ambiguity is the defect this version exists to remove.)
...
log "experiment 2: lost update at READ COMMITTED (asymmetric amounts) -- container $CONT"
log "reset schema (one-off psql, output verbatim below):"
docker exec -i "$CONT" psql -U postgres -a <<'SQL'
DROP TABLE IF EXISTS accounts;
CREATE TABLE accounts (id int PRIMARY KEY, balance int NOT NULL, last_writer text);
INSERT INTO accounts VALUES (1, 100, 'none');
...
NEW_A=$((VAL_A - 40)); NEW_B=$((VAL_B - 30))
log "client A read balance=$VAL_A, computes ${VAL_A}-40=${NEW_A} in the client"
log "client B read balance=$VAL_B, computes ${VAL_B}-30=${NEW_B} in the client"
log "correct serial outcome is 100-40-30 = 30, whatever order they run in"

feedA A3 "UPDATE accounts SET balance = $NEW_A, last_writer = 'A (withdrew 40)' WHERE id = 1;"; step
feedB B3 "UPDATE accounts SET balance = $NEW_B, last_writer = 'B (withdrew 30)' WHERE id = 1;"
...
SELECT * FROM accounts;
SQL
log "done. started at 100; A withdrew 40 and B withdrew 30, client-side, concurrently."
log "correct = 30. Anything else is a lost update, and last_writer names the write that survived."
