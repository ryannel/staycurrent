...
**Failure modes:** gate fails on the staged tree → halt (the design system's template: `Blocked` / `Cause` / `State` / `Action`, naming the exact missing artifact), staged set stays intact in `.staycurrent/staged/<slug>/`, `topics/<slug>/` is byte-identical to before, the session file is **not** cleared (resumable) — the skill fixes the named artifact in the staged tree and re-running `cut <slug>` is idempotent, since the gate re-inspects the same staged paths and an already-correct artifact simply passes. A crash between `executeCut`'s filesystem write and the CLI's git commit is the one real gap in this flow: the working tree could hold uncommitted mutations with no matching commit. Re-entry is still safe, in two branches. A **partial landing** (the crash hit mid-sync, before `article.md` — which lands last — was written) leaves the live tree reading as the previous version, so the monotonicity check permits the completing re-run: `cut <slug>` re-gates the intact staged tree and `executeCut` converges the sync. A **full landing without a commit** is the **converged re-entry**: the CLI detects that `topics/<slug>/` is already byte-identical to the staged tree, skips `executeCut` (whose monotonicity check would rightly refuse a same-version landing), and proceeds straight to the git commit and cleanup. Reserved-slug collision (a topic slug matching `skills`, `changelog`, `about`, `rss.xml`) and article/skill version-binding drift are both gate rejections, named the same way.
...
sequenceDiagram
    ...
    alt gate passes
        ...
        Core->>T: sync staged tree into topics/<slug>/ (status normalized to current; absent files removed -> CutReport.removed; no git)
...
sequenceDiagram
    ...
    Core->>St: seed prospective tree from live topic (staged baseline reads status: current)
    Core->>Aw: stamp status: current -> in-research (uncommitted working-tree edit, after seeding)
