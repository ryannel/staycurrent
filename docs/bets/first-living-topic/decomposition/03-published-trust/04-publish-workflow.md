...
- The export carries `CNAME` (`staycurrent.dev`) as an origin-consistency artifact agreeing with `site.config.json`'s `url` — the deployed origin and the canonical install one-liner name one domain. The custom domain itself binds through the repository's Pages settings and the operator's DNS records, not the file: Actions-based Pages deploys ignore `CNAME` (amended per the slice 3.4 review — GitHub's documented behaviour; the operator's manual step is surfaced at delivery).
