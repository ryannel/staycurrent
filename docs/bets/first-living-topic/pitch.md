...
- **Success Signal:** Three observations, each through the real front door, each falsifiable:
  ...
  2. The skill distribution mechanism works end to end: the install page renders with the canonical one-liner, the payload tree and zip are fetchable from the deployed site, and the placeholder skill is honestly labelled wherever a reader meets it (companion-skill authoring deferred by operator decision — change-proposal-2; stance-fidelity of an authored skill moves to the skill-design bet).
