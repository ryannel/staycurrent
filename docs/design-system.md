...
- **Superseded skill** (`/[topic]/skill` for an old version) — the install block is replaced (not disabled): "This skill renders **v3** of the stance. Install the current version instead →". The old artifact stays readable; the path forward is the emphasis.
